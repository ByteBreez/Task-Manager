...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: result.rows[0].id, email, username }, process.env.JWT_SECRET);
    res.json({ token });
  } catch (err) {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id, email: userEmail, username }, process.env.JWT_SECRET);
    res.json({ token });
  } catch (err) {
...
app.put('/api/tasks/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    io.emit('notification', `Task "${result.rows[0].title}" marked as ${status}`);
    res.json(result.rows[0]);
  } catch (err) {
...
cron.schedule('* * * * *', async () => {
  try {
    ...
    result.rows.forEach(task => {
      io.emit('notification', `Task "${task.title}" due in 1 hour: ${new Date(task.deadline).toLocaleString()}`);
    });
  } catch (err) {
