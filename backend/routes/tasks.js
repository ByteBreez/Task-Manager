  ...
  try {
    ...
    if (timeDifferenceMinutes > 0 && timeDifferenceMinutes < 15) {
      req.io.emit(
        ...
        `Reminder: Task "${
          newTask.title
        }" is due soon! Deadline: ${deadlineDate.toLocaleString()}`
      );
      await pool.query(
...
router.put("/:id", authenticateToken, async (req, res) => {
  const { id } = req.params;
  const { title, description, deadline, color, status, reminder_minutes } =
    req.body;
  try {
    ...
    const updatedTask = {
      ...
      reminder_minutes:
        reminder_minutes !== undefined
          ? reminder_minutes
          : currentTask.rows[0].reminder_minutes,
    };
